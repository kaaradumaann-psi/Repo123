...
import { canCreateRecord, createRecord } from '../records/recordStore';
import type { MMPIRecord } from '../records/recordStore';
import type { ScanSet } from '../scanner/pageSequence';
import { sortedPages } from '../scanner/pageSequence';

const today = () => new Date().toISOString().slice(0, 10);

type RecordCaptureProps = { definition: FormDefinition; scan: ScanSet; actor: AuthenticatedUser };

type Gender = 'Kadın' | 'Erkek' | 'Belirtmek istemiyor' | 'Diğer';

export function RecordCapture({ definition, scan, actor }: RecordCaptureProps) {
  ...
  const ready = actor.role === 'PSYCHOLOG' && canCreateRecord(pages, definition);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const record = createRecord({ client: { firstName, lastName, gender, age: Number(age), occupation, education, applicationDate, requestedBy } }, pages, definition, actor);
      setSaved(record);
  ...
  if (saved) return <section className="record-card record-success" aria-labelledby="record-success-title"><span className="record-success-icon" aria-hidden="true">✓</span><div><p className="auth-eyebrow">Kayıt tamamlandı</p><h3 id="record-success-title">Danışan ve ham cevaplar kaydedildi</h3><p>Kayıt ID'si: <strong className="record-id">{saved.id}</strong></p><p className="record-note">Bu sürüm klinik puanlama veya yorumlama yapmaz. Yeni kayıt için “Yeni set / sıfırla” ile yeni bir tarama oturumu başlatın.</p></div></section>;

  return <section className="record-card" aria-labelledby="record-title"><div className="record-heading"><div><p className="auth-eyebrow">Tarama sonrası kayıt</p><h3 id="record-title">Danışan bilgilerini tamamlayın</h3><p>Form seti hazır. Ham OMR cevapları değiştirilmeden bu bilgilerle birlikte saklanacak.</p></div><span className="record-ready">{pages.length} / {definition.totalPages} sayfa hazır</span></div>
    <form className="record-form" onSubmit={submit}>
      ...
      <div className="record-submit-row"><p>Kaydı oluşturan: <strong>{actor.firstName} {actor.lastName}</strong>. Cevap formatı ve özgün ölçümler korunur.</p><button className="record-submit" type="submit" disabled={busy}>{busy ? 'Kaydediliyor…' : 'Cevapları ve bilgileri kaydet'}</button></div>
    </form>
  </section>;
}
