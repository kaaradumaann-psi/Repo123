...
>
> **Mutabakat ölçümü (batch 28 · ayrıştırıcıya dokunulmadan, geçici kopya ile aynı 164 satır evreninde):** eşsiz **satır-etiketi = 148** · son-satır-durumuyla **VAR 61 + YOK 87 = 148 → kapalı** — ledger’in 148 iddiası satır-kümesinde doğrulanmıştır. Eşsiz **araç anahtarı = 152**: 29 satır-etiketi grup adlarına taşındı, 33 yeni grup anahtarı üretildi → 148 − 29 + 33 = 152 → **net +4, çok-kodlu satırların grup genişlemesidir (kalem tanımı farkı), sayım hatası değildir**. Kümülatif tabloda her blok satırı “VAR + YOK = Başlık”ı kapatır; **tek istisna Pd satırı** (20 ≠ 9 + 13 = 22 → **+2**); gerekçe dosyanın kendi notlarında: birleşik özet “Kodda VAR: 7 (… → kod kaydı olarak 9 kayıt)” ve ⚠️ bloğu “Başlık sayısı kendi batch deltasıyla, VAR/YOK kümülatif önceki satırdan taşınır”. Batch 18 FINAL notu ile ⚠️ satırlarının vaat ettiği yeniden sayım böylece ölçülmüştür; **hiçbir defter satırı değiştirilmedi** (DECISION-028).
> **Hüküm:** iki NOT (fark 4 / fark 2) yeniden sınıflandırma değil **kalem tanımı** gerektiriyordu; tanımlar ölçüldü → DECISION-031’in tek kalan adımı **gövde göçü**dür.
