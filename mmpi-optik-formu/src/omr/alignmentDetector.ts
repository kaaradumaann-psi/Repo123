...
/** Detects real dark connected components near QR predictions; never substitutes a predicted centre. */
export function detectAlignmentMarks(image: GrayImage, marks: readonly AlignmentMark[], qrTransform: Homography,
  qrCenter: Point): DetectedAlignmentMark[] | null {
  ...
  for (const mark of marks) {
    ...
    // QR extrapolation is least certain far from the small symbol. Search remains capped at 1 MP/mark.
    ...
    const candidates: (DetectedAlignmentMark & { cost: number })[] = [];
    for (let start = 0; start < visited.length; start++) {
      ...
      // Bounded observed-rectangle search: QR extrapolation is too uncertain to supply local edge angles.
      let squareFill = 0;
      ...
      for (let sy = -2; sy <= 2; sy++) for (let sx = -2; sx <= 2; sx++) {
        const probe = mapPoint(qrTransform, { x: mmCenter.x + sx * mark.width * .1, y: mmCenter.y + sy * mark.height * .1 });
    ...
    if (detected.some(previous => Math.hypot(previous.center.x - best.center.x, previous.center.y - best.center.y) < Math.sqrt(best.area))) return null;
