...
$diag$;
revoke all on function public.diag_report() from public;
grant execute on function public.diag_report() to anon, authenticated, service_role;
`;
...
async function catalogReport() {
  ...
  if (install.status !== 200 && install.status !== 201 && install.status !== 204) {
    record('warn', 'diag_report() kurulamadı', `${errLine(install)} — SQL endpoint kapalı olabilir`,
      'Dashboard → SQL Editor: supabase/migrations/20260920120000_repair_record_actions_and_audit.sql içeriğini çalıştırın');
    return null;
  }
  ...
  if (Array.isArray(report.migrations)) {
    const expected = ['20260915000000', '20260919000000', '20260919010000', '20260919020000', '20260920000000', '20260920120000'];
    const applied = report.migrations.map(String);
    const missing = expected.filter(prefix => !applied.some(version => version.startsWith(prefix)));
    if (missing.length === 0) record('ok', 'migration geçmişi', `6 migration da uygulanmış (${applied.join(', ')})`);
  ...
  if (report.audit_actor_nullable === true) {
    ...
    record('fail', 'audit_logs.actor',
      ...
      'supabase db push (20260920120000_repair_record_actions_and_audit.sql)');
  } else {
...
function summary() {
  ...
  if (has(/db push/i)) {
    ...
    steps.push('supabase db push                          # 6 migration’ın tamamı canlıya gider');
  }
...
async function main() {
  ...
  if (schemaOk) await catalogReport();
  await checkEdgeFunction();
  if (DESTRUCTIVE) await destructiveFlow();
  summary();
