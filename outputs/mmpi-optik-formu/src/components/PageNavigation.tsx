...
export function PageNavigation({ current, onChange }: { current: number; onChange: (index: number) => void }) {
  return <nav className="page-navigation" aria-label="Form sayfaları">
    ...
    <div className="page-list">{FORM_PAGES.map((page, index) => <button type="button" key={page.number}
      ...
      <span className="page-option-copy"><strong>Sayfa {String(page.number).padStart(2, '0')}</strong>
        <span>{page.firstItem}–{page.lastItem}. maddeler</span></span>
