...
test('566 unique item numbers, in column-first order, across four pages', () => {
  const numbers = FORM_PAGES.flatMap(page => page.columns.flatMap(column => column.items));
...
test('all pages share the same three-column, 48-row grid', () => {
  for (const page of FORM_PAGES) {
    ...
    for (const column of page.columns) assert.ok(column.items.length <= 48);
  }
  assert.deepEqual(FORM_PAGES[3]?.columns.map(column => column.items.length), [48, 48, 38]);
});
